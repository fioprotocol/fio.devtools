...
echo ------------------------------------------
dataJson="{
  \"payee_public_key\": \"FIO5oBUYbtGTxMS66pPkjC2p8pbA3zCtc8XD4dq9fMut867GRdh82\",
  \"amount\": 90000000000,
  \"max_fee\":4000000000,
  \"actor\": \"${fioactor}\",
  \"tpid\": \"adam@dapixdev\",
...
cmd="programs/clio/clio --no-auto-keosd --url http://$hostname:$nPort --wallet-url http://$hostname:$wPort  convert pack_action_data fio.token trnsfiopubky '${dataJson}'"
echo CMD: $cmd
...
cmd="./programs/clio/clio --no-auto-keosd --url http://localhost:8889  --wallet-url http://localhost:9899 sign '$unsignedRequest' -k 5J7ocyimcfcR6LMQmC43uo7JUBS5eRPbTwiJuH4qP77tToSZTVJ"
echo CMD: $cmd
