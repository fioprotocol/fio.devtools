...
rm -f ./container/etc/config.ini

echo
read -p $'\n1. V1 History Node 2. State History Node \nChoose(#):' hChoice

if [[ $hChoice == 1 || $hChoice == 2 ]]; then

  if [[ $hChoice == 1 ]]; then
    cp ./container/etc/config.ini-v1history ./container/etc/config.ini
  else
    #hChoice == 2
    cp ./container/etc/config.ini-statehistory ./container/etc/config.ini
  fi
  ...
  echo
    ...
    echo "Unknown option: $hChoice! Exiting..."
    exit 1
fi
