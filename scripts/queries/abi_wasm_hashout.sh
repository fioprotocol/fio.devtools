...
do_compare_abiwasm_hashout() {
  ...
  echo -e "\nSelect Exclude Input Option (<Enter> to include all inputs):"
  read -p $'1. Exclude LocalNet 2. Exclude File\nChoose(#):' mChoice
  mChoice=${mChoice:-0}
...
while getopts 'cmoirxh' opt; do
  case "$opt" in
    c)
      echo
      echo "Generating hashes of each type (file, *net), for side-by-side comparison..."
      echo
      echo "Note: The ABI hashes are generated by pre-processing the input to remove non-"
      echo "      essential elements for the purposes of hash comparison."
      echo
      echo "      !!!The ABI hashes are NOT to be used as truth for Local/TestNet/MainNet!!!"
      echo
    ...
    m)
      echo "Generating hashes for each network (LocalNet, TestNet, MainNet) in markup format..."
      DO_HASH_GEN_MARKUP=true
      ;;

    o)
      echo "Generating hashes for each network (LocalNet, TestNet, MainNet)..."
      DO_HASH_GEN=true
      ;;

    i)
      echo
      echo -e "\tABI/WASM Hash comparison involves the following process:"
      echo -e "\t  ABI;"
      echo -e "\t    clio is used to download the ABI and store in a local file"
      echo -e "\t    jq is used to remove any undesired elements which have been"
      echo -e "\t        deemed non-essential for commparison purposes"
      echo -e "\t    openssl is used to generate a SHA256 hash"
      echo -e "\t    The hashes are compaired using basic string equality"
      echo -e "\t  WASM;"
      echo -e "\t    clio is used to download the WASM hash directly"
      echo -e "\t    the hashes are compaired using basic string equality"
      echo
      echo -e "\t  Local file and network input (localhost, TestNet, MainNet) is"
      echo -e "\t  considered."
      echo
      echo -e "\tABI/WASM Hash output is basically the same as above without any"
      echo -e "\tpre-processing to remove elements (input is taken as-is) and output"
      echo -e "\tto the screen. Only network input (localhost, TestNet, MainNet) is"
      echo -e "\tconsidered."
      echo
      echo -e "\tExecute this script ($(basename $0)) directly with the '-h' or"
      echo -e "\t'-r' arg for more info."
      echo
      ;;
    ...
    r)
      ...
      echo
      echo "ABI/WASM Information:"
      echo "Note: The ABI must be retrieved and stored locally to generate the hash, whereas the WASM hash"
      echo "      may be retrieved directly."
      echo
      echo "Commands:"
      ...
      echo "Cmd to generate ABI hash:  openssl sha256 ${devtools_dir}/bin/localnet.abi"
      echo "Cmd to pre-process abi:    jq -c -S '(.)' ${devtools_dir}/bin/localnet.abi"
      echo
      ;;
    ...
    ?|h)
      ...
      echo -e "\t    Hashes are generated from normalized input (trivial differences are"
      echo -e "\t    removed) and are for comparison purpose only (not for publication)!"
      echo
      echo -e "-m\tGenerate hashes for each network (localhost, TestNet, MainNet) in markup format"
      echo -e "\tusing the commands clio -u <URL> get abi <Contract>,"
